...
export function generatePayslipPDF(payslip: any) {
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text(payslip.status.toUpperCase(), 176, 22.5, { align: 'center' });
  ...
  doc.text(`Department: ${payslip.department}`, 14, 64);
  doc.text(`Position: ${payslip.job_position}`, 14, 71);
  if (payslip.bank_account_no) doc.text(`Bank Account: ${payslip.bank_account_no}`, 14, 78);
