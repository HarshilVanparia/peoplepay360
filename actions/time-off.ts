...
import { syncEmployeeLeaveStatuses } from './employees'
...
export async function approveTimeOffRequest(requestId: string, employeeId: string, leaveTypeId: string, durationDays: number) {
  await transaction(async (conn) => {
    ...
    const [types] = await conn.execute(
      `SELECT requires_allocation FROM leave_types WHERE id = ?`,
      [leaveTypeId]
    ) as any[]

    if (types[0]?.requires_allocation) {
      const allocation = (await conn.query(
        `SELECT id FROM leave_allocations WHERE employee_id = ? AND leave_type_id = ? AND status = 'Approved' AND valid_from <= ? AND valid_to >= ? AND remaining_days >= ? ORDER BY valid_to ASC LIMIT 1 FOR UPDATE`,
        [employeeId, leaveTypeId, request.start_date, request.end_date, durationDays]
      ) as any)[0][0]

      if (!allocation) throw new Error('No approved leave balance can cover this request.')
      await conn.execute(
        `UPDATE leave_allocations SET taken_days = taken_days + ? WHERE id = ?`,
        [durationDays, allocation.id]
      )
      await conn.execute(
        `UPDATE leave_requests SET status = 'Approved', allocation_id = ?, approved_at = NOW() WHERE id = ?`,
...
export async function getMyLeaveContext() {
  ...
  if (!employeeId) throw new Error('You must be signed in.')
  return query(
    `SELECT t.id, t.name, t.payroll_treatment, COALESCE(SUM(a.remaining_days), 0) AS remaining_days
     FROM leave_types t
     LEFT JOIN leave_allocations a ON a.leave_type_id = t.id AND a.employee_id = ? AND a.status = 'Approved'
     GROUP BY t.id, t.name, t.payroll_treatment
     ORDER BY t.name`,
    [employeeId]
  )
}
