...
import { ChevronRight, Send, Calculator, CheckCircle2, Banknote, AlertTriangle, FileText } from 'lucide-react';
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {['Draft', 'Computed'].includes(payrun.status) && (
              <form action={async () => { 'use server'; await recomputePayrun(String(id)); }}>
                <button className="inline-flex items-center gap-2 rounded-xl border border-violet-500/40 bg-violet-900/10 px-4 py-2.5 text-sm font-bold text-violet-300 hover:bg-violet-900/30 transition-all">
            ...
            {payrun.status === 'Computed' && (
              <form action={async () => { 'use server'; await updatePayrunStatus(String(id), 'Validated', (payslips as any[]).map(p => p.id)); }}>
                <button className="inline-flex items-center gap-2 rounded-xl border border-blue-500/40 bg-blue-900/10 px-4 py-2.5 text-sm font-bold text-blue-300 hover:bg-blue-900/30 transition-all">
            ...
            {payrun.status === 'Validated' && (
              <form action={async () => { 'use server'; await updatePayrunStatus(String(id), 'Paid', (payslips as any[]).map(p => p.id)); }}>
                <button className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-900/30">
                  <Banknote size={15} /> Mark Paid
                </button>
              </form>
            )}
            {/* Send payslips */}
            <form action={async () => { 'use server'; await distributePayslips(String(id)); }}>
              <button className="inline-flex items-center gap-2 rounded-xl border border-slate-600 px-4 py-2.5 text-sm font-semibold text-slate-300 hover:border-slate-400 hover:text-white transition-all">
              ...
              <tr key={p.id} className="hover:bg-violet-900/10 transition-colors">
                ...
                <td className="py-3.5 px-5">
                  <div className="flex justify-end items-center gap-2">
                    ...
                    {p.status === 'Computed' && (
                      <form action={async () => { 'use server'; await updatePayrunStatus(String(id), 'Validated', [p.id]); }}>
                        <button className="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-violet-500 transition-all">Validate</button>
                      </form>
                    )}
                    {p.status === 'Validated' && (
                      <form action={async () => { 'use server'; await updatePayrunStatus(String(id), 'Paid', [p.id]); }}>
                        <button className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-500 transition-all">Mark Paid</button>
                      </form>
                    )}
                    {p.status === 'Paid' && (
