...
export default async function PayrollDashboard() {
  ...
  if (role === 'Employee') {
    ...
    return <div className="space-y-7"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs tracking-[.2em] text-violet-300">MY WORKSPACE</p><h1 className="text-3xl font-bold">Welcome back {session?.user?.name}</h1><p className="mt-2 text-sm text-slate-400">Your current leave balance and attendance summary.</p></div><Link href="/attendance" className="rounded-lg bg-violet-600 px-4 py-3 text-sm font-bold">Open my attendance</Link></div><div className="grid md:grid-cols-3 gap-5">{balances.map(balance=><div key={balance.name} className="rounded-2xl border border-slate-200 p-6"><p className="text-sm text-slate-400">{balance.name}</p><p className="mt-3 text-4xl font-bold">{Number(balance.remaining).toFixed(0)} <span className="text-base text-slate-400">days</span></p></div>)}<div className="rounded-2xl border border-slate-200 p-6"><p className="text-sm text-slate-400">This month</p><p className="mt-3 text-4xl font-bold">{attendance[0]?.present || 0}</p><p className="text-sm text-slate-400">attendance records</p></div></div></div>;
  }
